...
export function SubmitDmpId() {
  ...
  const { lastMessage } = useWebSocket(`${import.meta.env.VITE_BACKEND_WS_PROTOCOL}://${import.meta.env.VITE_BACKEND_DOMAIN}:${import.meta.env.VITE_BACKEND_PORT}`,
    {
  ...
  const { mutate } = useMutation<void, unknown, FormValues>({
    mutationFn: (values) => {
      return axios
        .post(`${import.meta.env.VITE_BACKEND_HTTP_PROTOCOL}://${import.meta.env.VITE_BACKEND_DOMAIN}:${import.meta.env.VITE_BACKEND_PORT}/dmp/id`,
          {
            dmpId: values.dmpId,
          }
        )
  ...
  return (
    <>
      <Row className="mb-4">
        <Col md={8}>
          <div className="mt-2">
            This form demonstrates a simple implementation of the DMSP AI Tool backend service. When a valid DMP ID is
            submitted, the service will fetch that plan, extract its content, and query the AI Tool to review the plan.
          </div>
